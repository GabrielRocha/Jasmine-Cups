...
*/
  ...
  function jas_getHostTotalPages($host){
    if (!$host){
      $source="jas_getHostTotalPages";
      $message="Missing host !";
      $hint="Please specify \$host for this function.";
...
//      return DB_Dump_Result($result);
	...
	function jas_getHostLastJobs($host, $days, $printerName=""){
    if (!$host || !$days){
      $source="jas_getHostLastJobs";
      $message="Missing host and/or number of days !";
      $hint="Please specify \$host and/or \$days for this function.";
      ER_Handler::getInstance()->logCrit($source, $message, $hint);
      return false;
    }
    // Clean the input variables and prepare query
...
//      return DB_Dump_Result($result);
	...
	/* jas_getHostRankings: Returns an associative array containing
	...
	function jas_getHostRankings($count, $threshold=0, $printerName="", $dpt=""){
     if (!$count){
    ...
    if($result=DB_query($query)){ //Assignment !
      ...
      $tableUR->setCaption("Host que mais imprimiram");
      $tableUR->setColumns(array('IP', 'Total'));
      ...
      mysql_free_result($result);
      return $tableUR->displayTable('10');
    }
    else{
      $source="jas_getUserRankings";
      $message="Query failed !";
  ...
  function jas_getUserRankings($count, $threshold=0, $printerName="", $dpt=""){
    ...
    if($result=DB_query($query)){ //Assignment !
      ...
      $tableUR=new TBL_table();
      $tableUR->setCaption("Usuários que mais imprimiram");
  ...
  function jas_getPrinterRankings($count, $dpt=""){
    ...
    if($result=DB_query($query)){ //Assignment !
      ...
      $tablePR->setCaption("Impressoras mais usadas");
  ...
  function jas_searchObject($string, $objectType){
    ...
    if (!strcmp($objectType,"'user'") || !strcmp($objectType, "'printer'") || !strcmp($objectType, "'server'") || !strcmp($objectType,"'host'")){
      $source="jas_searchObject";
      $message="Wrong object type !";
      $hint="Please choose either 'printer', 'server', 'user' , or 'host' for \$objectType.";
      ER_Handler::getInstance()->logCrit($source, $message, $hint);
      return false;
    }
    ...
    $queryField=(!strcmp($objectType,"user"))?"user":(!strcmp($objectType,"printer"))?"printer":(!strcmp($objectType,"host"))?"host":"server";
    switch ($objectType) {
      case "user":
        $queryField="user";
        break;
